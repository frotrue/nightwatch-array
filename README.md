...
현재 진행은 **별자리 연구 95개 → 같은 하늘의 M31 첫 관측 → 모듈 연구·구매·편성**이다.
모듈은 성도에서 구매하고 연구탭의 편성 팝업에서 장착한다. 총 5종, 최대 5자리이며
처음 2자리가 열린다. 현재 값은 [모듈 상세 기준](docs/design-details.md#m31과-모듈)에 있다.
...
빠른 게이트 14개를 통과하면 Windows 실행 파일을 갱신한다.
