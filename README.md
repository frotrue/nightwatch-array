...
현재 진행은 **기존 별자리 연구 95개 → 같은 성도의 바깥 별자리 10개·연구별 47개**다.
모듈 지원은 페가수스·도마뱀자리 2개에 모으고, 나머지 8개는 수동 추적·훑기·반향·접시·M31 등
...
공개하며 건너뛰기가 가능하다. 모듈 14종을 모두 같은 확률로 뽑는다. 중복 보유·장착을 허용하고 같은 모듈의 증감분은 합산한다.
