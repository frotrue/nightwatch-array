...
기존 별자리 연구 95개를 완료하면 성도가 축소되고 **확장 관측 성도**가 이어진다.
안드로메다는 별도 스테이지로 이동하지 않고 기존 유성 하늘에서 관측하는 새 표적이다.
유성·장비·좌클릭 관측·회차·정산은 그대로 유지된다.

**안드로메다 첫 관측 → 성도에서 모듈 구매 → 별도 팝업에서 장착**이 첫 후속 구간이다.
집중·광역 모듈을 성도의 해당 노드에서 각각 1억 2천만 데이터로 구매하면 보관함에
추가된다. 자동 장착은 하지 않는다. 연구탭 안의 **모듈 편성** 버튼으로
고리 편성 팝업을 연다. 보관함 클릭으로 시계 방향 첫 빈 자리에 장착하고, 고리의
모듈 클릭으로 해제한다. 처음에는 다섯 자리 중 두 자리가 열리며 안드로메다
연구로 나머지 세 자리를 순서대로 연다. 팝업에는 보유 모듈만 표시하며
구매·시험 화면은 없다. 편성은 무료이고 기존 연구와 함께 실제 관측에 적용된다.
...
확장 성도의 축소된 별자리 모양을 누르면 기존 성도를 다시 확대할 수 있다.
과거 국부은하군 29개 노드는 복구하지 않았다. 그중 기능 정의 12개와 이전 엔딩은
호환·기존 진단용으로 남아 있다. 추가 천체·연구 가지·최종 엔딩은 첫 구간 검토 후
확장할 계획이며 현재 구현 범위에 포함하지 않는다.
