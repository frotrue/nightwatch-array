...
현재 진행은 **별자리 연구 95개 → 같은 성도의 새 별자리 4개·연구별 18개**다.
페가수스·백조·세페우스·독수리자리의 별을 길게 눌러 모듈과 특이 관측을 연구한다.
같은 하늘의 M31 첫 관측으로 후속 연구와 특수 유성이 열린다.
모듈 14종을 연구·구매·특수 유성 표본 뽑기로 얻고 편성 팝업에서 장착한다.
처음 2자리에서 최대 5자리로 확장하며 중복 보유·장착이 가능하다. 같은 모듈의 증감분은
합산한다. [특수 유성과 모듈 뽑기](docs/expansion-design.md)에 현재 값이 있다.
...
| E | 진행·저장을 바꾸지 않는 이전 카탈로그 엔딩 미리 보기. 다시 누르면 복귀 |
| Backspace | 런 리셋 |
