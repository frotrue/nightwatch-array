...
현재 진행은 **기존 별자리 연구 95개 → 같은 성도의 바깥 별자리 12개·연구별 60개**다.
모듈 지원은 페가수스·도마뱀자리 2개에 모으고, 나머지 10개는 천체 해금과 수동 추적·훑기·공명·접시 등
...
| 키 | 동작 |
|---|---|
| D / N / A | 데이터 +100 / 구매 가능한 첫 기본 연구 / 기존·확장 전체 155개 연구 해금 |
...
화이트홀은 디버그 소환으로 기능을 검증하는 시제품이다. 정한 방향으로 이동하며
수동·자동 관측 완료 시 1.4초 동안 양극 제트로 유성 24개를 방출하고 사라진다.
동시에 최대 3개, 미관측 기본 수명 16초이며 회차 종료·리셋·불러오기 때 정리한다.
방출 유성은 기존 관측과 보상을 따른다. 자연 출현·연구 해금은 아직 추가하지 않았다.
