...
은하 지도 해금 뒤 **은하 선택** 화면에서 안드로메다를 선택하고 **관측하러 가기**로
새 관측 무대에 들어간다. 기존 은하 연구 노드·장식 표식·설치 정보는 제거했다.
현재 목적지는 안드로메다 하나이며, 기존 별자리 성도는 별도 보기 버튼으로 열 수 있다.
별 무리·G1 구상성단·NGC 206 별 구름을 관측해 기존 데이터를 모으고,
`U`의 **모듈 구매 / 편성**에서 집중·광역 관측 모듈을 각각 1억 2천만 데이터로
구매한다. 슬롯은 하나이며 편성 변경은 무료다. 첫 구매는 자동 장착한다.
기존 연구의 관측 반경·원거리 분석 속도·데이터 배율과 접시의 기본 표적 지원을
사용한다. 회차 종료 때 모듈 화면에서 다음 관측으로 진행하며 구매·편성·진행은
기존 저장 슬롯에 함께 보존된다. 5~10개 은하 전체를 완성한 버전은 아니다.

기존 별자리 연구 95개를 바탕으로 은하별 관측 단계를 확장하는 중이다. 과거
국부은하군 기능 연구 12개와 장식 기록 17개는 저장 호환·기존 진단용 데이터로
남아 있지만 은하 선택 화면의 구매 대상은 아니다. 새로운 단계 전체와 최종 엔딩은
아직 설계 중이다. 2시간 이상은 장기 목표이며 현행 완주 시간이나 경제 밸런스의
승인값은 아니다.
