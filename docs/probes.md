...
| `smoke_test.gd` | `SMOKE_TEST_PASS` | Main loop, saves, localization, held tracking/sweep, input/modals/settings, round accounting; en/ko tutorial pointer pass-through from the first observation to held research purchase, completion and skip; immediate echo unlock/legacy channels, additive dish forecasts, screen-space survey counts and manual-only spectral rewards |
...
| Sample meteor material and accessibility | `meteor_art_review.gd -- --special --animate` on Vulkan and OpenGL; `SAMPLE_METEOR_ART_REVIEW_PASS`. Captures native/detail motion, completion, warning, body tint, reduced effects and full fade under `build/sample_meteor_review/after/<renderer>/`; checks frozen pixels at zero motion and one-time sample credit through the real completion path. `--before` captures the old art without the new surface checks. Synthetic save-isolated poses. |
