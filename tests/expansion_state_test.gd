...
func _initialize() -> void:
	...
	model.grant_copy("long_baseline")
	_check(model.owned_count("long_baseline") == 2 and model.equip("long_baseline", 0) and model.equip("long_baseline", 1), "two copies equip together")
	_check(is_equal_approx(model.effect("new_speed"), 0.7) and is_equal_approx(model.stacked_effect("long_baseline", "baseline_speed"), 2.2), "same module bonuses and penalties add")
	model.unlocked_slots = 5
	_check(not model.equip("long_baseline", 2), "cannot equip a third unowned copy")
	var saved: Dictionary = JSON.parse_string(JSON.stringify(model.get_save_data()))
	model.load_save_data(saved)
	_check(model.owned_count("long_baseline") == 2 and model.installed_count("long_baseline") == 2, "quantity and duplicate slots survive JSON")
	model.load_save_data({"purchased": ["long_baseline"], "slots": ["long_baseline", "long_baseline"]})
	_check(model.owned_count("long_baseline") == 1 and model.installed_count("long_baseline") == 1, "legacy inventory means one copy")
