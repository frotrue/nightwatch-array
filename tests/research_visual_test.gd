...
func _run() -> void:
	...
	if failures.is_empty():
		print("RESEARCH_VISUAL_PASS: 95 base and 60 extension bindings, state-ink hierarchy, star/cluster/galaxy draw routing and bilingual tutorial truth")
		quit(0)
	else:
...
func _test_live_binding(game) -> void:
	...
	var branches := {}
	var kinds := {}
	_check(Balance.UPGRADE_NODES.size() == 95 and chart.node_hold_bars.size() == 155, "binding gate covers 95 base and 60 extension research markers")
